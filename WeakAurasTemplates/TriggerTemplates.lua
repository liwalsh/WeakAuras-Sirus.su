...
local function subTypesFor(item, regionType)
  ...
  local icon = {
    target = function()
      ...
      t1:SetTexture(select(3,GetSpellInfo(41241)));
  ...
  if (item.type == "ability") then
    tinsert(types, {
      fallback = true,
      icon = select(3,GetSpellInfo(icon.cd)),
    ...
    if (item.charges) then
      ...
      tinsert(types, {
        icon = select(3,GetSpellInfo(icon.cd2)),
      ...
      elseif(item.totem) then
        tinsert(types, {
          icon = C_Spell.GetSpellIcon(icon.cd2),
  ...
  elseif(item.type == "buff") then
    ...
    tinsert(types, {
      icon = C_Spell.GetSpellIcon(icon.cd),
...
function WeakAuras.CreateTemplateView(Private, frame)
  ...
  local function createSortFunctionFor(table)
    return function(a, b)
      if not table[a].title or not table[b].title then return false end
  ...
  local function replaceTriggers(data, item, subType)
    ...
    for child in WeakAuras.TraverseLeafsOrAura(data) do
  ...
  createButtons = function(selectedItem) -- selectedItem is either a regionType or a trigger section
    ...
    if (not newView.data) then
      ...
      if TemplatePrivate.Private.regionOptions then
  ...
  function newView.Open(self, data, targetId)
    ...
    newView.spec = C_Talent.GetSpecInfoCache().activeTalentGroup
